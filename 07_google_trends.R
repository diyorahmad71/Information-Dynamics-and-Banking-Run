...
# Look in data/ first, then the project root, so the files are found in
# either location (load_csv in 02_data_download.R does the same).
find_gt <- function(fname) {
  cand <- c(file.path("data", fname), fname)
  hit  <- cand[file.exists(cand)]
  if (length(hit)) hit[1] else cand[1]
}
csv_svb_path     <- find_gt("google_trends_svb.csv")
csv_bankrun_path <- find_gt("google_trends_bankrun.csv")
